...
from HB873_drpdrc import *


def execute_ckt_case_study(logger, target_file: str) -> None:
    target_loads_list = random.sample(range(1, 9), 8)  # list with random load's numbers
    ev_shapes = random.sample(list(range(1, 5000)), 8)  # select randomly a specific shape
    ...
    for voltvar in ('OFF', 'ON'):
        ...
        for percentage in percentages_list:
            ...
            if n:
                for i in range(1, n + 1):
                    ...
                    dss.Text.Command(f'new load.ev{i} phases=1 '
                                     f'bus1=HB873RES{target_loads_list[i - 1]}.{phase_a}.{phase_b} '
                    ...
                    if voltvar == 'ON':
                        # dss.Text.Command(f'new Storage.VE{i} bus1=CA744RES{target_loads_list[i - 1]}.{phaseA}.{phaseB} '
            ...
            dss.Text.Command(f'new energymeter.busbar_PV_VE_percentage_{percentage}_voltvar_{voltvar} '
                             f'element=transformer.HB873 terminal=1')
...
def create_results_folder() -> None:
    for word in ('_0', '_20', '_40', '_60', '_80', '_100'):
        if not os.path.exists(Path(f'../../results/HB873_results/voltvar_off/pv_ev{word}').resolve()):
            os.makedirs(Path(f'../../results/HB873_results/voltvar_off/pv_ev{word}').resolve())

    for word in ('_20', '_40', '_60', '_80', '_100'):
        if not os.path.exists(Path(f'../../results/HB873_results/voltvar_on/pv_ev{word}').resolve()):
            os.makedirs(Path(f'../../results/HB873_results/voltvar_on/pv_ev{word}').resolve())
...
def move_files_results(_folder: str) -> None:
    ...
    for file in _files:
        ...
        destination = '../../results/HB873_results'
    ...
    shutil.copy(Path(_folder + '/losses_voltvar_OFF.csv').resolve(),
                Path('../../results/HB873_results/').resolve())
