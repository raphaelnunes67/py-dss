...
class CA744Simulation:
    def __init__(self, logger):
        self.percentages_list = [0, 20, 40, 60, 80, 100]
        self.ev_shapes = random.sample(list(range(1, 5001)), 46)
        self.target_loads_list = random.sample(range(1, 47), 46)  # list with random load's numbers
        self.logger = logger
                ...
                if n:
                    for i in range(1, n + 1):
                        ...
                        dss.Text.Command(f'new load.ev{i} phases=1 '
                                         f'bus1=CA744RES{self.target_loads_list[i - 1]}.{phase_a}.{phase_b} '
...
if __name__ == '__main__':
    log = Log()
    logger = log.set_logger_stdout('CA744_simulation')
